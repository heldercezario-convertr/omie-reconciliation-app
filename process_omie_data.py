...
import streamlit as st # Importar streamlit para usar st.error e st.code
import traceback # Importar traceback para detalhes do erro
...
def process_omie_spreadsheet(file_path):
    try:
        ...
        # Refinada: Uma transação é 'Recebido/Pago' se 'A_Pagar_ou_Receber' for 0 (ou muito próximo de zero) OU se 'Conciliado' for 'Sim'
        # E se o 'Valor_da_Conta' for maior que zero (para evitar contas zeradas)
        df['Status_Conciliacao'] = df.apply(lambda row: 'Recebido/Pago' if ((row['A_Pagar_ou_Receber'] <= 0.01 and row['Valor_da_Conta'] > 0) or row['Conciliado'] == 'Sim') else 'Pendente', axis=1)


        # Criar coluna de tipo de transação mais amigável
        df['Tipo_Transacao'] = df['Tipo'].map({'1. Contas a Receber': 'Contas a Receber', '2. Contas a Pagar': 'Contas a Pagar'}).fillna('Não Identificado')
